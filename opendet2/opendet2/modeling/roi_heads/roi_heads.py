...
class DropoutStandardROIHeads(OpenSetStandardROIHeads):
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            predictions = [
                self.box_predictor(box_features, testing=True)
...
@ROI_HEADS_REGISTRY.register()
class SIRENCFRPNOpenSetStandardROIHeads(ROIHeads):
    @configurable
    def __init__(
        self,
        *,
        ...
        if self.training:
            ...
            if len(proposals):
                ...
                if (
                    ...
                    selected_fg_samples = (
                        (gt_classes != predictions[0].shape[1] - 1).nonzero().view(-1)
                    )
                    ...
                    for index in range(self.num_classes):
                        if index == 0:
                            ...
                            mean_embed_id = torch.cat(
                                (
                                    mean_embed_id,
                                    self.data_dict[index].mean(0).view(1, -1),
                                ),
                                0,
                            )
                    ...
                    if len(ood_samples) != 0:
                        ...
                        energy_score_for_fg = self.log_sum_exp(
                            predictions[0][selected_fg_samples][:, :-1], 1
                        )
                        ...
                        if False:
                            ...
                            print(
                                torch.cat(
                                    (
                                        F.sigmoid(
                                            output[: len(selected_fg_samples)]
                                        ).view(-1, 1),
                                        torch.ones(len(selected_fg_samples), 1).cuda(),
                                    ),
                                    1,
                                )
                            )
                            print(
                                torch.cat(
                                    (
                                        ...
                                        torch.zeros(len(ood_samples), 1).cuda(),
                                    ),
                                    1,
                                )
                            )
                        else:
                            weights_fg_bg = torch.Tensor(
                                [
                                    len(selected_fg_samples) / float(len(input_for_lr)),
                                    len(ood_samples) / float(len(input_for_lr)),
                                ]
                            ).cuda()
                            criterion = (
                                torch.nn.CrossEntropyLoss()
                            )  # weight=weights_fg_bg)
                            output = self.logistic_regression(input_for_lr.view(-1, 1))
...
class OpensetROIHeadsLogisticGMMNewLimitFPEnergy(ROIHeadsLogisticGMMNew):
    ...
    def forward(
        ...
        if self.training:
            ...
            if iteration >= self.start_iter:
                goc_samples = self.sample_localboxes_and_completeboxes_t2(
                    proposals, targets
                )
            else:
                goc_samples = None
    ...
    def export_pos_box(self, proposal_boxes, iou_with_gt, match_gtid, gt_num, e2):
        ...
        for gt_id in range(gt_num):
            ...
            x = np.random.choice(
                len(propid_matched_thisgt[0]), int(len(propid_matched_thisgt[0]) / 2)
            )
            y = np.random.choice(
                len(propid_matched_thisgt[0]), int(len(propid_matched_thisgt[0]) / 2)
            )
    ...
    def export_neg_box(
        self, gt_boxes, proposal_boxes, iou_with_gt, match_gtid, gt_num, e2
    ):
        p1 = 0.2
        g1 = 0.2
        ...
        for gt_id in range(gt_num):
            ...
            neg_proposals.append(
                neg_proposal_boxes[propid_matched_thisgt][include_more_gt_index]
            )

            pro_ratio_thisgt = pro_ratio[gt_id, propid_matched_thisgt[0]]
            include_more_pro_index = torch.where(pro_ratio_thisgt >= p1)
            neg_proposals.append(
                neg_proposal_boxes[propid_matched_thisgt][include_more_pro_index]
            )
    ...
    def sample_localboxes_and_completeboxes_t2(self, proposals, targets):
        ...
        for i, img in enumerate(proposals):
            ...
            proposal_boxes = proposal_boxes[
                potential_index
            ]  # 更新proposals, iou, match_gt_id
            iou_with_gt = iou_with_gt[potential_index]
            match_gtid = match_gtid[potential_index]

            pos_x_proposals, pos_y_proposals, pos_bool_list = self.export_pos_box(
                proposal_boxes, iou_with_gt, match_gtid, gt_num, e2
            )
            neg_proposals = self.export_neg_box(
                gt_boxes, proposal_boxes, iou_with_gt, match_gtid, gt_num, e2
            )
    ...
    def loss_local_complete_t2(self, goc_samples, features):
        ...
        loss_pos1 = (1 - bool_batch) * F.relu(
            complete_scores_x + 1e-2 - complete_scores_y
        ) + (bool_batch) * F.relu(complete_scores_y + 1e-2 - complete_scores_x)
        MSELoss = nn.MSELoss()
        loss_pos2 = MSELoss(
            complete_scores_x, torch.ones_like(complete_scores_x)
        )  # F.relu(-1 * torch.log(complete_scores_x + 0.0001))
        loss_pos3 = MSELoss(
            complete_scores_y, torch.ones_like(complete_scores_y)
        )  # F.relu(-1 * torch.log(complete_scores_y + 0.0001))
        ...
        loss_neg = F.relu(
            complete_scores_neg + 1e-3 - minscores
        )  # MSELoss(complete_scores_neg, torch.zeros_like(complete_scores_neg))#F.relu(complete_scores_neg)

        pos_weight = 1 if bool_batch.shape[0] > 0 else 0
        neg_weight = 1 if complete_scores_neg.shape[0] > 0 else 0
        loss = (
            pos_weight * (loss_pos1.mean() + loss_pos2.mean() + loss_pos3.mean())
            + loss_neg.mean() * neg_weight
        )
        # print("shape: {}  {}   ".format(bool_batch.shape[0], complete_scores_neg.shape[0]))
        print(
            "minscores:{:.4f} | loss_pos1.mean:{:.4f} | loss_pos2.mean():{:.4f} | loss_pos3.mean:{:.4f} | loss_neg.mean:{:.4f}".format(
                minscores,
                loss_pos1.mean(),
                loss_pos2.mean(),
                loss_pos3.mean(),
                ...
                if (
                    ...
                    for index in indices_numpy:
                        ...
                        self.data_dict[dict_key] = torch.cat(
                            (
                                self.data_dict[dict_key][1:],
                                box_features[0][index].detach().view(1, -1),
                            ),
                            0,
                        )
                elif (
                    ...
                    if len(ood_samples) != 0:
                        # add non-object
                        selected_non_obj_samples = (
                            (gt_classes == predictions[0].shape[1] - 1)
                            .nonzero()
                            .view(-1)
                        )
                        energy_score_for_non_obj = self.log_sum_exp(
                            predictions[0][selected_non_obj_samples][:, :-1], 1
                        )
                        sample_index_non_obj, sample_weight_non_obj = (
                            self.sample_non_obj(energy_score_for_non_obj)
                        )
                        energy_score_for_non_obj = energy_score_for_non_obj[
                            sample_index_non_obj
                        ]
                        non_obj_loss = (
                            F.relu(energy_score_for_non_obj).mean() * 0.5
                        )  # 1e-3
                        ...
                        if False:
                            ...
                            output = self.logistic_regression(input_for_lr.view(-1, 1))
                            lr_reg_loss = (
                                criterion(output, labels_for_lr.long()) + non_obj_loss
                            )
                    ...
                    for index in indices_numpy:
                        ...
                        if self.number_dict[dict_key] < self.sample_number:
                            self.data_dict[dict_key][self.number_dict[dict_key]] = (
                                box_features[0][index].detach().unsqueeze(0)
                            )
                            self.number_dict[dict_key] += 1
                # create a dummy in order to have all weights to get involved in for a loss.
                loss_dummy = self.cos(
