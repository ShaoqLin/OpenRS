...
class AugInput:
    ...
    * The input must have a ``transform(tfm: Transform) -> None`` method which
    ...
    def transform(self, tfm: Transform) -> None:
        ...
        if hasattr(tfm, "mutation_rate"):
            self.image = tfm.apply_noise(self.image, self.boxes).numpy()
            # print('noise added')
            # arrayimage = self.image / 255.0
            # plt.imsave(f"{self.image.size}_noise_box.png", arrayimage)
